...
uint32_t FlashDestination = StartWriteFlashAddress; /* Flash user program offset */
...
//////////////////////////
uint8_t test_temp;
HAL_StatusTypeDef test_status_flag = HAL_BUSY;
...
static  int32_t Receive_Byte (uint8_t *c, uint32_t timeout)
{
  ...
  //status_flag = HAL_UART_Receive(&huart1,c, 1, 0x3E8);
    ...
    while(1)
    {
        test_status_flag = HAL_UART_Receive(&huart1,&test_temp, 1, 0x3E8);
    }
    #endif
    ...
    {
        /* 0x3E8 = 1000 ms = 1s */
        if (HAL_UART_Receive(&huart1,c, 1, 0x3E8) == HAL_OK)
        {
          return 0;
        }
    }
