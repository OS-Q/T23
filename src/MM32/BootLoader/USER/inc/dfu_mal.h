...
#define INTERNAL_FLASH_BASE 0x08000000	//ÄÚ²¿FLASH
...
extern uint16_t MAL_Init (void);
extern uint16_t MAL_Erase (uint32_t SectorAddress);
extern uint16_t MAL_Write (uint32_t SectorAddress, uint32_t DataLength);
extern uint8_t  *MAL_Read (uint32_t SectorAddress, uint32_t DataLength);
extern uint16_t MAL_GetStatus(uint32_t SectorAddress ,uint8_t Cmd, uint8_t *buffer);
